...
TOKEN = "8503702501:AAFp9aExYlQmjllu7IrLMOeraWZfY7U7s2w"
CHAT_ID = "7342381562"
...
async def get_eur_usd_price():
    async with aiohttp.ClientSession() as session:
        async with session.get("https://api.coingecko.com/api/v3/simple/price?ids=usd&vs_currencies=eur") as r:
            data = await r.json()
            return 1 / float(data["usd"]["eur"])

async def send_alert(context: ContextTypes.DEFAULT_TYPE, msg):
    await context.bot.send_message(chat_id=CHAT_ID, text=msg)

async def check_alerts(context: ContextTypes.DEFAULT_TYPE):
    price = await get_eur_usd_price()
    for level, active in list(alert_state.items()):
        if active:
            level = float(level)
            if price >= level and "above" in active_alerts:
                await send_alert(context, f"🚨 EUR/USD above {level}! Current: {price:.5f}")
            elif price <= level and "below" in active_alerts:
                await send_alert(context, f"🚨 EUR/USD below {level}! Current: {price:.5f}")
...
async def above(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(f"✅ EUR/USD alert set: above {level}")
    except:
...
async def below(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(f"✅ EUR/USD alert set: below {level}")
    except:
